...
{
    [TestClass] public class Test_Parser_Enumeration
    {
        ...
        private void TEST()
        {
            string metadataName = "Перечисление.Перечисление1"; //"Перечисление.СтавкиНДС";
            ...
            {
                ShowMetadataObject((Enumeration)@object);
            }
        }
        private void ShowMetadataObject(Enumeration @object)
        {
            ...
            foreach (EnumValue value in @object.Values)
            {
                Console.WriteLine($" - {value.Name} {{{value.Uuid}}} {value.Alias}");
            }
        }
